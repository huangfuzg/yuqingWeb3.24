...
//                    return "" + $scope.orderBy;
                    ...
                    if(!template) {
                        template =
                            '<ul {{options.ulClass}} class="pn5">' +
                            '<li ng-repeat="node in node.{{options.nodeChildren}} | filter:filterExpression:filterComparator {{options.orderBy}}" ng-class="[headClass(node)]" {{options.liClass}}' +
                            'set-node-to-data>' +
                            '<span class="tree-branch-head" ng-class="iBranchClass()" ng-click="selectNodeHead(node)"></span>' +
                            '<span class="tree-leaf-head {{options.iLeafClass}}"><i class="file"></i></span>' +
