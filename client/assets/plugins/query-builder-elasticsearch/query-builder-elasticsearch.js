...
(function(root, factory) {
    ...
    QueryBuilder.defaults({
        ESBoolOperators: {
            ...
            in :              function(v){if((typeof v) == "string"){v = v.split(',');}return v.map(function(e) { return e.trim();}); },
            not_in:           function(v){ if((typeof v) == "string"){v = v.split(',');}return v.map(function(e) { return e.trim();}); },
        ...
        getESBool: function(data) {
            ...
            return (function parse(data) {
                ...
                data.rules.forEach(function(rule) {
                    ...
                    function get_baseTypeValue(baseType,rule,value)
                    {
                        if(rule.data)
                        {
                            if(rule.data.type)
                            {
                                if(Object.keys(rule.data).length > 1)
                                {
                                    var ret = JSON.parse(JSON.stringify(rule.data));
                                    // ret.add = function(data)
                                    // {
                                    //     if(data.type)
                                    //         delete data.type;
                                    ...
                                    //     }
                                    // }
                                    if(rule.data.type == "term" || rule.data.type == "terms" || rule.data.type == "ids" || rule.data.type == "prefix" || rule.data.type == "fuzzy" || rule.data.type == "wildcard" || rule.data.type == "regexp")
                                    {
                                        ret.value = value;
                                    }
                                    else if(rule.data.type == "query_string" || rule.data.type == "common" || rule.data.type == "match" || rule.data.type == "match_phrase" ||rule.data.type == "match_phrase_prefix" || rule.data.type == "multi_match" || rule.data.type == "simple_query_string")
                                    {
                                        ret.query = value;
                                    }
                                    console.log(ret);
                    ...
                    function get_baseType(rule){
                        if(rule.data)
                        {
                            if(rule.data.type)
                            {
                                if(rule.data.type == "term" || rule.data.type == "terms" || rule.data.type == "range" || rule.data.type == "query_string" || rule.data.type == "common" || rule.data.type == "match" || rule.data.type == "match_phrase" ||rule.data.type == "match_phrase_prefix" || rule.data.type == "multi_match" || rule.data.type == "simple_query_string" || rule.data.type == "ids" || rule.data.type == "prefix" || rule.data.type == "fuzzy" || rule.data.type == "wildcard" || rule.data.type == "regexp")
                                {
                                    return rule.data.type;
                                }
                            }
                        }
                        return getQueryDSLWord(rule);
                    }
                    ...
                    function make_query(rule) {
                        ...
                        if (ope.nb_inputs !== 0) {
                            ...
                            es_key_val[rule.field] =  get_baseTypeValue(baseType,rule,mdb.call(that, get_value(rule)));
                            // part[getQueryDSLWord(rule)] = es_key_val;
                            var baseType = get_baseType(rule);
                            part[baseType] = es_key_val; 
                        }
